#!/bin/bash
# Parse a whole directory using OpenNLP
# Directory should contain preprocessed texts, with one sentence per line

if [ $# -lt 3 ]
then
    echo "Usage: parse_dir.sh <input-dir> <output-dir> <processes> <file-list-file>"
    exit 1
fi
...
for file_list in $split_dir/process-*; do
	...
	(
		...
		while read input_file; do
			...
			echo -n "." >&2
		done <$file_list | ./run cam.whim.opennlp.Parse ../models/opennlp/en-parser-chunking.bin
	) &
done
